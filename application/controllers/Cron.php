...
class Cron extends CI_Controller
{
    ...
    {     
        ...
        //$query = "SELECT * FROM amaz_aug WHERE (tracking=1 OR stock_noti='true') AND requires_rescrape > -1  LIMIT 0, 5";
        ...
        //$query = "SELECT * FROM amaz_aug WHERE (tracking=1 OR stock_noti='true') AND requires_rescrape > -1 and asin='B07XYQYXD3'";
        //9944975016
        ...
        foreach ($trackingAsins as $previousScrape) {
            ...
            //$seller_name = $previousScrape->seller_name;
...
//	print_r($user);
	...
	if($sellerStockChange  ){
	    ...
	    if ($sellerStockChange && !$firstStatusChange) {
            ...
            $previousScrape->seller_name = $user->company;
                // uncomment the following to send notification
            $this->notifySeller($user, $notificationData, $previousScrape,$stockChange );
            }
        } else {
            //echo "else<br>";
            $this->common_model->updateData(
        ...
        if($amazonStockChange  ){
            //echo "test<br>";
                ...
                $stockChange = $amazonInStock;
            $this->common_model->insertData('notification', $notificationData);
            //$firstStatusChange = $previousScrape->amznotseller === null
                //    || $previousScrape->amznotseller === '';
                $firstStatusChange = $previousScrape->amazonstock === null
                || $previousScrape->amazonstock === '';
            //echo "first===".$firstStatusChange."<br>";
            //if ($amazonStockChange && !$firstStatusChange) {
            if ($amazonStockChange && !$firstStatusChange) {
                $previousScrape->seller_name = 'Amazon';
                    // uncomment the following to send notification
    ...
    protected function notifySeller($user, $notificationData, $previousScrape, $stockChange)
    {
        echo "notifySeller \n";
	if ($user && $user->global_noti == 'true') {
		$this->common_model->insertData('notification', $notificationData);
            if ($previousScrape->email_noti == 'true') {
                $this->notifyByEmail($user, $notificationData, $previousScrape, $stockChange);
            }
    ...
    protected function notifyByEmail($user, $notificationData, $previousScrape, $stockChange)
    {
        echo "notifyByEmail \n";
        print_r($previousScrape);
	...
	//if ($notificationData['amznotseller'] == '0'){
            if ($stockChange == '1' && ($previousScrape->stock_noti == 'true' || $previousScrape->stock_noti == 1)) {
		    $message = $previousScrape->seller_name." back in stock on " . $date->format('m/d/Y h:iA').$m;
		    $mail_subject = $notificationData['asin']." - $previousScrape->seller_name Back In Stock";
            } else if ($stockChange == '0' && $previousScrape->tracking == 1) {
		    $message = $previousScrape->seller_name." ran out of stock on " . $date->format('m/d/Y h:iA').$m;
		    $mail_subject = $notificationData['asin']." - $previousScrape->seller_name Ran Out of Stock";
            }
        //}
