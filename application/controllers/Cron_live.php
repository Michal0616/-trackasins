...
class Cron extends CI_Controller
{
    ...
    {     
        ...
        // $query = "SELECT * FROM amaz_aug WHERE (tracking=1 OR stock_noti='true') AND requires_rescrape > -1 and asin='B00U2PNI7U'";
        $trackingAsins = $this->db->query($query)->result();
        // echo '<pre>';print_r($trackingAsins);
        //$trackingAsins = $this->db->query("SELECT * FROM amaz_aug WHERE asin='B0714DS829'")->result();       
	    ...
	    //$previousScrape->asin='B00IN8OJ58';
	    //$previousScrape->asin='B082MSYGVR';
	    //$getstock = get_stock_new1($previousScrape->asin, $seller_name);
        $getstock = getStockDetail($previousScrape->asin, $seller_name);
        echo "result======\n";
	    print_r($getstock);
...
//echo "$$$".$sellerstock."***".$amznotseller;
	...
	//if ($notificationData['amznotseller'] == '0'){
        ...
        if ($message) {
            //echo "sending===";
            $this->sendEmail($user, $message, $notificationData, $mail_subject);
        }
    }
    ...
    protected function sendEmail($user, $message, $notificationData, $mail_subject)
    {
        ...
        $email = $user->notification_email ? $user->notification_email : $user->email;
        //$email = "gpwebsoft.karthick@gmail.com";
